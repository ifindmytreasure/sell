/**
 * Created by BlueMelancholy on 2019/2/1/001.
 */
$(function () {
    var loading = false;
    ...
    var searchDivUrl = '/o2o/fronted/listshopdetailpageinfo?shopId=' + shopId;
    // 渲染出店铺基本信息以及商品类别列表以供搜索
    getSearchDivData();
    // 预先加载10条商品信息
    addItems(pageSize, pageNum);
    // 给兑换礼品的a标签赋值兑换礼品的URL，2.0讲解
    $('#exchangelist').attr('href', '/o2o/fronted/awardlist?shopId=' +
        shopId);
    ...
    function getSearchDivData() {
        ...
        $.getJSON(url, function (data) {
            if (data.success) {
                var shop = data.shop;
                $('#shop-cover-pic').attr('src', getImgUrl() + shop.shopImg);
            ...
            if (data.success) {
                ...
                data.productList.map(function (item, index) {
                    html += '' + '<div class="card" data-product-id='
                        ...
                        + getImgUrl() + item.imgAddr + '" width="44">' + '</div>'
    ...
    $('#records').attr('href', '/o2o/fronted/myrecord?shopId=' +
        shopId);
    $('#points').attr('href', '/o2o/fronted/mypoint?shopId=' + shopId);

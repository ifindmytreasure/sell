...
public class UserAwardMapServiceImpl implements UserAwardMapService {
	...
	@Override
	@Transactional
	public UserAwardMapExecution addUserAwardMap(UserAwardMap userAwardMap) throws UserAwardMapOperateException {
				...
				if (userAwardMap.getPoint() != null && userAwardMap.getPoint() > 0) {
					...
					if (userShopMap != null) {
						...
						if (userShopMap.getPoint() >= userAwardMap.getPoint()) {
							...
							if (effectedNum <= 0) {
								throw new UserAwardMapOperateException("更新积分信息失败");
							}
						} else {
							throw new UserAwardMapOperateException("积分不足无法领取");
						}
					} else {
						// 在店铺没有积分，则抛出异常
						throw new UserAwardMapOperateException("在本店铺没有积分，无法对换奖品");
					}
				}
				// 插入礼品兑换信息
				effectedNum = userAwardMapDao.insertUserAwardMap(userAwardMap);
				if (effectedNum <= 0) {
					throw new UserAwardMapOperateException("领取奖励失败");
				}
				return new UserAwardMapExecution(UserAwardMapStateEnum.SUCCESS, userAwardMap);
			} catch (Exception e) {
				throw new UserAwardMapOperateException("领取奖励失败:" + e.toString());
			}
		} else {
	...
	@Override
	@Transactional
	public UserAwardMapExecution modifyUserAwardMap(UserAwardMap userAwardMap) throws UserAwardMapOperateException {
		if (userAwardMap == null && userAwardMap.getUsedStatus() == null && userAwardMap.getUserAwardId() == null) {
			...
			try {
				...
				throw new UserAwardMapOperateException("modifyUserAwardMap error: " + e.getMessage());
			}
		}
	}
}

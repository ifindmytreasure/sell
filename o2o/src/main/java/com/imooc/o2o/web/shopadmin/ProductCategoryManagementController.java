...
public class ProductCategoryManagementController {
	...
	@ResponseBody
	public Result<List<ProductCategory>> getProductList(HttpServletRequest request) {
	...
	public Map<String, Object> addProductCategorys(@RequestBody List<ProductCategory> productCategoryList, HttpServletRequest request) {
		...
		if (productCategoryList != null && productCategoryList.size() > 0) {
			...
			} catch (ProductCategoryOperateException e) {
				modelMap.put("success", false);
	...
	private Map<String, Object> removeProductCategory(Long productCategoryId, HttpServletRequest request) {
		...
		if (productCategoryId != null && productCategoryId > 0) {
			try {
				...
				System.out.println("删除结果状态：" + pe.getState());
				System.out.println("成功状态:" + ProductCategoryStateEnum.SUCCESS.getState());
				if (pe.getState() == ProductCategoryStateEnum.SUCCESS.getState()) {
					modelMap.put("success", true);
				} else {
					modelMap.put("success", false);

...
public class LocalAuthController {
	...
	private Map<String, Object> bindLocalAuth(HttpServletRequest request) {
		...
		if (userName != null && password != null && user.getUserId() != null && user != null) {
	...
	private Map<String, Object> changLocalPwd(HttpServletRequest request) {
		...
		// 从session中获取当前用户信息(用户一旦通过微信登录之后，便能获取到用户的信息)
		PersonInfo user = (PersonInfo) request.getSession().getAttribute("user");
		if (userName != null && password != null && newPassword != null && user != null && user.getUserId() != null && !newPassword.equals(password)) {
			try {
			...
			} catch (LocalAuthOperateException e) {
				modelMap.put("success", false);
			...
			modelMap.put("success", false);
			modelMap.put("errMsg", "输入的信息不能为空，且旧密码和新密码不能相同");
		}
		return modelMap;
	}
